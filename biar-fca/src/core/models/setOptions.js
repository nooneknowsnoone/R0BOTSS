...
async function setOptions(globalOptions, options = {}) {
    const optionHandlers = {
        ...
        logging(value) {
            ...
            utils.logOptions(value);
        },
        debug(value) {
            globalOptions.debug = value;
            utils.setDebugOptions(value);
        },
        maxAttachmentBytes(value) {
            if (Number.isFinite(value) && value > 0) {
                globalOptions.maxAttachmentBytes = Number(value);
            } else {
                delete globalOptions.maxAttachmentBytes;
            }
        },
        uploadFallback(value) {
            globalOptions.uploadFallback = value;
        },
        mediaPreprocessor(value) {
            globalOptions.mediaPreprocessor = typeof value === "function" ? value : null;
        },
        tempDir(value) {
            if (typeof value === "string" && value.trim()) {
                globalOptions.tempDir = value;
            } else {
                delete globalOptions.tempDir;
            }
        },
        mqttReconnectPolicy(value) {
            globalOptions.mqttReconnectPolicy = value && typeof value === "object" ? { ...value } : {};
        },
