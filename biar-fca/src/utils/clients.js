...
function parseAndCheckLogin(ctx, http, retryCount = 0) {
  ...
  return async (data) => {
    ...
    if (data.statusCode !== 200) {
      const requestUri = data.request?.uri;
      const requestUrl = requestUri ? `${requestUri.protocol}//${requestUri.hostname}${requestUri.pathname}` : "unknown-url";
      const error = new Error(`Request failed with status code ${data.statusCode} at ${requestUrl}.`);

      error.statusCode = data.statusCode;
      error.res = data.body;
      error.url = requestUrl;
      error.error = `Request failed with status code ${data.statusCode}.`;

      if (data.statusCode === 413) {
        error.error = "Attachment upload too large.";

        if (requestUrl.includes("/ajax/mercury/upload.php")) {
          error.message = "Facebook rejected the attachment upload with status code 413. The file is too large to send.";
        } else {
          error.message = `Request failed with status code 413 at ${requestUrl}.`;
        }
      }
