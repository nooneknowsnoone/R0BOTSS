...
function setDebugOptions(value) {
  debugOptions = value;
}
function shouldDebug(category) {
  if (debugOptions === true) return true;
  if (!debugOptions) return false;
  if (Array.isArray(debugOptions)) return debugOptions.includes(category) || debugOptions.includes("*");
  if (typeof debugOptions === "object") return Boolean(debugOptions[category] || debugOptions["*"]);
  return false;
}
function log(...args) {
...
function debug(category, ...args) {
  ...
  if (!shouldDebug(category)) return;
  console.debug(ws, chalk.cyan.bold("[DEBUG]"), `[${category}]`, ...args);
}
